  ...
  return (
    <div className="space-y-6 text-left">
      <div className="bg-[var(--color-bg)] rounded-[var(--radius-panel)] border border-[var(--color-border)] p-6 sm:p-8 space-y-6 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          {billing.hasCustomer && (
            <button
              ...
              disabled={isPending}
              className="px-4 py-2 border border-[var(--color-border-strong)] hover:bg-[var(--color-bg-subtle)] text-[var(--color-text)] text-xs font-bold rounded-[var(--radius-control)] transition-all cursor-pointer disabled:opacity-50"
            >
        ...
        <div className="flex items-center justify-center gap-3 py-2 bg-[var(--color-bg-subtle)] rounded-[var(--radius-card)] border border-[var(--color-border)] w-fit mx-auto px-4">
          <button
            type="button"
            onClick={() => setCycle("monthly")}
            className={`px-4 py-1.5 text-xs font-bold rounded-[var(--radius-control)] transition-all cursor-pointer ${
              cycle === "monthly" ? "bg-[var(--color-bg)] text-[var(--color-text-heading)] shadow-sm" : "text-[var(--color-text-muted)]"
            }`}
          >
            Cobrança Mensal
          </button>
          <button
            ...
            className={`px-4 py-1.5 text-xs font-bold rounded-[var(--radius-control)] transition-all cursor-pointer flex items-center gap-1.5 ${
              cycle === "yearly" ? "bg-[var(--color-bg)] text-[var(--color-text-heading)] shadow-sm" : "text-[var(--color-text-muted)]"
            }`}
          >
            <span>Cobrança Anual</span>
            <span className="text-[var(--text-2xs)] font-semibold text-[var(--color-success)] bg-[var(--color-success-light)] px-1.5 py-0.5 rounded-[var(--radius-sm)]">
              Desconto
            </span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          {billing.plans.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`p-5 rounded-[var(--radius-card)] border transition-all space-y-4 flex flex-col justify-between ${
                  isCurrentPlan
              ...
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {isCurrentPlan && (
                      <span className="px-2 py-0.5 bg-[var(--color-primary)] text-white font-semibold text-[var(--text-2xs)] rounded-full uppercase">
                        Plano Atual
                      </span>
                    )}
                  </div>
                  ...
                  <div className="pt-2">
                    <span className="text-xl font-semibold text-[var(--color-text-heading)]">
                      {formatMoney(price, billing.currency)}
                    </span>
                    <span className="text-[var(--text-2xs)] text-[var(--color-text-subtle)]">/{cycle === "yearly" ? "ano" : "mês"}</span>
                  </div>
                </div>
                ...
                {billing.isOwner && (
                  <button
                    ...
                    className={`w-full py-2.5 rounded-[var(--radius-control)] font-bold text-xs shadow-xs transition-all cursor-pointer disabled:cursor-not-allowed ${
                      isCurrentPlan
