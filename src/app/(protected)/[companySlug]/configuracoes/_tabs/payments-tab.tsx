      ...
      <div className="bg-white rounded-3xl border border-[var(--color-border)] p-6 sm:p-8 space-y-6 shadow-xs">
        <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[var(--color-success-light)] text-[var(--color-success)] flex items-center justify-center shrink-0 border border-[var(--color-success-border)]">
              <CreditCard className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base font-extrabold text-[var(--color-text-heading)]">Métodos de Pagamento Internacionais & Regionais</h2>
          ...
          <label className="flex items-center gap-2 bg-[var(--color-bg-subtle)] border border-[var(--color-border)] px-3.5 py-2 rounded-xl text-xs font-bold text-[var(--color-text-heading)] cursor-pointer shadow-2xs">
            <input
              type="checkbox"
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono font-bold bg-[var(--color-bg-muted)] text-[var(--color-text-heading)] px-2 py-0.5 rounded-full">
                      {m.region}
                    </span>
                  </div>
                ...
                {m.enabled && m.accountDetails !== undefined && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-[var(--color-border)] rounded-xl px-3 py-2 text-xs font-medium text-[var(--color-text-heading)] focus:ring-2 focus:ring-[var(--color-success)]"
                    />
                  </div>
                )}
              </div>
