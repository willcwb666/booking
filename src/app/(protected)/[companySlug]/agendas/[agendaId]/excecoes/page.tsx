    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-2xl">
      <div className="mb-6">
        <Link
          ...
          className="text-sm text-[var(--color-text-muted)] hover:text-[var(--color-text)] mb-3 block"
        >
          ‹ Agendas
        </Link>
        <h1 className="text-xl font-bold text-[var(--color-text-heading)]">{agenda.name}</h1>
        <p className="text-sm text-[var(--color-text-muted)] mt-0.5">Exceções de agenda — dias bloqueados ou com horário diferente</p>
      </div>
      <ExcecoesClient
