...
export function ResetPresetModal({ company, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--color-navy)]/60 backdrop-blur-xs animate-fadeIn">
      <div className="bg-white rounded-3xl border border-[var(--color-border)] p-6 sm:p-8 max-w-md w-full shadow-2xl space-y-6 text-left">
        <div className="space-y-2">
          <div className="w-12 h-12 rounded-2xl bg-amber-100 text-amber-700 flex items-center justify-center text-xl font-bold">
            🔄
          </div>
          <h2 className="text-lg font-extrabold text-[var(--color-text-heading)]">Resetar Catálogo de Serviços?</h2>
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button
            ...
            onClick={onClose}
            disabled={isPending}
            className="px-4 py-2.5 bg-[var(--color-bg-muted)] hover:bg-[var(--color-bg-muted)] text-[var(--color-text)] font-bold text-xs rounded-xl transition-all cursor-pointer disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
